...
public class CommandHelp implements ICommand
{
	...
	public boolean execute(User user, TextChannel channel, String command, List<String> parameters) throws Exception
	{
		...
		output.add("To view information about a polyphasic sleep schedule, type `" + NapBot.CONFIGURATION.messagePrefix + "` followed by the schedule name. For example, `" + NapBot.CONFIGURATION.messagePrefix + "DC1` will display information about DC1.");
		output.add("");
		...
		output.add("-----------------------------------------------");
		output.add("**To go back to monophasic:** Type `" + NapBot.CONFIGURATION.messagePrefix + "set monophasic`.");
		output.add("-----------------------------------------------");
		...
		output.add("-----------------------------------------------");
		output.add("**To create a new napchart:** Type `" + NapBot.CONFIGURATION.messagePrefix + "create` followed by a series of time ranges. For example, `" + NapBot.CONFIGURATION.messagePrefix + "create 03:00-05:00 08:00-08:20 14:00-14:20 21:00-23:00`. A napchart link will then be generated for you.");
		output.add("-----------------------------------------------");
		channel.sendMessage(StringUtils.join(output, '\n')).complete();
		return true;
	}
