...
public class CommandVeteranList implements ICommand
{
	@Override
	public String[] getCommandName()
	{
		return new String[] { "veteranlist" };
	}
	...
	{
		return "list everyone who is considered a veteran";
	}
	...
	public boolean execute(User user, TextChannel channel, String command, List<String> parameters, Message message) throws Exception
	{
		...
		for (Member m : mlist)
		{
			if (!m.getUser().isBot())
			{
				...
				for (Role r : roles)
				{
					if (r.getName().equalsIgnoreCase("Veteran"))
					{
		...
		String currentMessage = "There are **" + strings.size() + "** members on this server who are considered veteran polyphasers:\n" + StringUtils.join(strings, ", ");
